...
export default function StockBatchesIndex({
    ...
    function handleDelete(batch: StockBatch) {
        if (
            confirm(
                `Delete this batch? Only possible if nothing has been sold from it yet.`,
            )
        ) {
    ...
    const filtered = useMemo(() => {
        ...
        return batches.data.filter((batch) => {
            const matchesQuery =
                !q ||
                [batch.product?.name, batch.supplier?.name].some((f) =>
                                            ...
                                            >
                                                ...
                                                <td className="p-3 text-right">
                                                    <Button
                                                        ...
                                                        className="size-8 text-red-600 opacity-80 transition-opacity group-hover:opacity-100 hover:bg-red-50 hover:text-red-700 dark:hover:bg-red-950/40"
                                                        title="Delete batch"
                                                    >
