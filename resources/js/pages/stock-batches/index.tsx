...
// The backend sends full ISO timestamps for received/expiry dates even though
// only the date matters here — rendering them raw (as before) blew the table
// out to roughly double the width it needed. This always renders a short,
...
export default function StockBatchesIndex({ batches, isOwner }: { batches: PaginatedBatches; isOwner: boolean }) {
    ...
    function toggleExpiryFilter(value: typeof expiry) {
        setExpiry((current) => (current === value ? 'all' : value));
    }
    ...
    const filtered = useMemo(() => {
        ...
        if (sortLowestFirst) {
            return [...rows].sort((a, b) => a.remaining_qty - b.remaining_qty);
        }

        return rows;
    }, [batches.data, query, expiry, sortLowestFirst]);
                ...
                {!isEmpty && (
                    <>
                        {/* Stats — double as filters; tap one to filter, tap again to clear.
                            4 cards now (Depleted added), so the grid needs 4 columns on larger
                            screens instead of 3 — 3 columns with 4 items orphaned the last card
                            onto its own row. */}
                        <div className="grid grid-cols-2 gap-2.5 sm:max-w-2xl sm:grid-cols-4 sm:gap-3">
                            ...
                            <StatCard
                                ...
                                accent="amber"
                                active={expiry === 'soon'}
                                onClick={() => toggleExpiryFilter('soon')}
                            />
                            <StatCard
                                icon={XCircle}
                                label="Expired"
                                ...
                                onClick={() => toggleExpiryFilter('expired')}
                            />
                            <StatCard
                                icon={PackageX}
                                label="Depleted"
                                value={depletedCount}
                                accent="gray"
                                active={expiry === 'depleted'}
                                onClick={() => toggleExpiryFilter('depleted')}
                            />
                        </div>
                        ...
                        <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center">
                            ...
                            <button
                                type="button"
                                onClick={() => setSortLowestFirst((v) => !v)}
                                aria-pressed={sortLowestFirst}
                                className={cn(
