...
export default function AccountsPayableIndex({
    ...
    const [choosingPaymentFor, setChoosingPaymentFor] = useState<number | null>(
        null,
    );

    function markPaid(payable: Payable, method: 'cash' | 'gcash') {
        router.post(`/accounts-payable/${payable.id}/mark-paid`, {
            ...
            <div className="p-4">
                <div className="mb-4 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            What's owed to suppliers — created automatically
                            when stock is received on credit.
                        </p>
                    </div>
                </div>
                ...
                {totalUnpaid > 0 && (
                    <div className="mb-4 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm dark:bg-amber-950/20">
                        <span className="font-medium">{peso(totalUnpaid)}</span>{' '}
                        owed to suppliers, unpaid
                    </div>
                )}
                            ...
                            {payables.map((p) => (
                                <tr key={p.id} className="border-t">
                                    ...
                                    <td className="p-3 text-right">
                                        {!p.is_paid &&
                                            choosingPaymentFor !== p.id && (
                                                <Button
                                                    size="sm"
                                                    onClick={() =>
                                                        setChoosingPaymentFor(
                                                            p.id,
                                                        )
                                                    }
                                                >
