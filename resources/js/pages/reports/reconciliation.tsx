...
export default function Reconciliation({
    ...
    const hasVariance =
        r.shrinkage_variance !== null && Math.abs(r.shrinkage_variance) >= 1;
    ...
    return (
        <>
            ...
            <div className="space-y-6 p-4">
                ...
                {!r.reconciliation_complete && (
                    <div className="rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm dark:bg-amber-950/20">
                        ...
                        <p className="text-muted-foreground">
                            This needs a finalized physical inventory count
                            before {start}, and another on or before {end}, to
                            establish Beginning and Ending Inventory values. The
                            figures below for Revenue, actual COGS, and Expenses
                            are still accurate on their own — only the periodic
                            cross-check and shrinkage variance need both counts.
                        </p>
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                    <div className="rounded-lg border p-4">
                        ...
                        <p className="text-lg font-semibold">
                            {peso(r.beginning_inventory)}
                        </p>
                        {r.beginning_inventory_date && (
                            <p className="mt-1 text-xs text-muted-foreground">
                                as of {r.beginning_inventory_date}
                            </p>
                        )}
                    </div>
                    <div className="rounded-lg border p-4">
                    ...
                    <div className="rounded-lg border p-4">
                        ...
                        <p className="text-lg font-semibold">
                            {peso(r.ending_inventory)}
                        </p>
                        {r.ending_inventory_date && (
                            <p className="mt-1 text-xs text-muted-foreground">
                                as of {r.ending_inventory_date}
                            </p>
                        )}
                    </div>
                    <div className="rounded-lg border p-4">
                        <p className="text-xs text-muted-foreground">
                            Implied COGS (periodic)
                        </p>
                        <p className="text-lg font-semibold">
                            {peso(r.implied_cogs)}
                        </p>
                ...
                {hasVariance && (
                    <p className="text-xs text-muted-foreground">
                        The shrinkage variance is the gap between what the
                        periodic formula (Beginning + Purchases − Ending)
                        implies should have left inventory, and what your sales
                        actually recorded as COGS. A positive number means more
                        inventory disappeared than sales account for — likely
                        theft, damage, spoilage, or a miscount. It is not yet
                        folded into Net Profit above; investigate it, then
                        record it as an Expense once confirmed so future reports
                        reflect it.
                    </p>
                )}
            </div>
