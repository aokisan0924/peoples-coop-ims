...
export default function SalesIndex({ sales }: { sales: PaginatedSales }) {
    ...
    const voidedCount = useMemo(
        () => sales.data.filter((s) => s.voided_at).length,
        [sales.data],
    );
    const completedCount = sales.data.length - voidedCount;
    const cashCount = useMemo(
        () => sales.data.filter((s) => s.payment_method === 'cash').length,
        [sales.data],
    );
    ...
    const filtered = useMemo(() => {
        ...
        return sales.data.filter((sale) => {
            const matchesQuery =
                ...
                sale.cashier.name.toLowerCase().includes(q);
            const matchesStatus =
                status === 'all' ||
                (status === 'voided' ? !!sale.voided_at : !sale.voided_at);
    ...
    const hasFilters =
        query.trim() !== '' || status !== 'all' || payment !== 'all';
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="mt-0.5 text-sm text-muted-foreground">
                            Browse transactions, review payments, and void sales
                            when needed.
                        </p>
                    </div>
                </div>
                        ...
                        <div className="flex items-start gap-2.5">
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="truncate font-mono text-xs text-muted-foreground">
                                    {voidingSale.receipt_number}
                                </p>
                            </div>
                        </div>

                        <p className="mt-3 text-xs text-muted-foreground">
                            A reason is required and will be recorded with this
                            sale.
                        </p>
                        <Input
                            value={reason}
    ...
    return (
        <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed py-16 text-center">
            ...
            <p className="max-w-xs text-sm text-muted-foreground">
                Transactions will show up here once sales are recorded at the
                register.
            </p>
        </div>
    );
