...
export default function LocationsIndex({ locations: locationList }: { locations: Location[] }) {
    ...
    const activeCount = useMemo(() => locationList.filter((l) => l.is_active).length, [locationList]);
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            {!isEmpty && (
                                <Badge className="border-0 bg-[var(--pos-green)]/15 font-normal text-[var(--pos-green)]">{locationList.length}</Badge>
                            )}
                        </div>
                        <p className="mt-0.5 text-sm text-muted-foreground">Manage your store locations and their availability.</p>
                    </div>
                ...
                {!isEmpty && (
                    <>
                        ...
                        <div className="grid grid-cols-3 gap-2.5 sm:max-w-xl sm:gap-3">
                            <StatCard icon={Building2} label="Total" value={locationList.length} accent="teal" />
                            <StatCard icon={CheckCircle2} label="Active" value={activeCount} accent="green" />
                ...
                {isEmpty ? (
                    ...
                    <>
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {filtered.map((location) => (
                                        <tr key={location.id} className="group border-t transition-colors hover:bg-muted/30">
                                            <td className="p-3 font-medium">
                                                <div className="flex items-center gap-2.5">
                                                    ...
                                                    {location.is_main && (
                                                        <Badge className="border-0 bg-[var(--pos-teal)] font-normal text-white">Main</Badge>
                                                    )}
                                                </div>
                                            </td>
                                            <td className="max-w-xs truncate p-3 text-muted-foreground">{location.address ?? '—'}</td>
                                            <td className="p-3 text-muted-foreground tabular-nums">{location.phone ?? '—'}</td>
                                            <td className="p-3">
                                            ...
                                            <td className="p-3 text-right">
                                                <div className="inline-flex gap-1.5 opacity-80 transition-opacity group-hover:opacity-100">
                                                    ...
                                                    >
                                                        <Link href={locations.edit(location.id).url}>
                        ...
                        <div className="grid grid-cols-1 gap-2.5 sm:hidden">
                            {filtered.map((location) => (
                                <div key={location.id} className="rounded-xl border bg-card p-3 shadow-sm">
    ...
    return (
        <Badge
            className={cn(
                ...
                active ? 'bg-[var(--pos-green)]/15 text-[var(--pos-green)]' : 'bg-red-100 text-red-700 dark:bg-red-950/50 dark:text-red-400',
            )}
        >
...
function EmptyState() {
    return (
        <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed py-16 text-center">
            ...
            <p className="max-w-xs text-sm text-muted-foreground">Add your first branch to start managing its inventory and staff.</p>
...
function NoResults({ query, onClear }: { query: string; onClear: () => void }) {
    return (
        <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed py-16 text-center">
            ...
            <p className="max-w-xs text-sm text-muted-foreground">Try a different name, address, or phone number.</p>
