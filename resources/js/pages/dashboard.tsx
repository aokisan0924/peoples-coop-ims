...
export default function Dashboard({
    ...
    const [trendDays, setTrendDays] =
        useState<(typeof TREND_RANGES)[number]>(30);
            ...
            <div className="mx-auto max-w-[1600px] space-y-5 p-3 sm:space-y-6 sm:p-6">
                ...
                {canManage && monthProfitLoss && (
                    <div className="rounded-xl border bg-card p-4 shadow-sm">
                        ...
                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
                            <PlStat
                                label="Revenue"
                                value={monthProfitLoss.revenue}
                            />
                            <PlStat
                                label="COGS"
                                value={monthProfitLoss.cogs}
                                negative
                            />
                            <PlStat
                                label="Expenses"
                                value={monthProfitLoss.expenses}
                                negative
                            />
                            <PlStat
                                label="Gross Profit"
                                value={monthProfitLoss.gross_profit}
                                sub={`${monthProfitLoss.gross_margin_pct}% margin`}
                            />
                            <PlStat
                                label="Net Profit"
                ...
                {canManage && (
                    <div className="grid grid-cols-1 gap-3 sm:gap-4 lg:grid-cols-3">
                        <BreakdownPanel
                            icon={
                                <Banknote className="size-4 text-[var(--pos-teal)]" />
                            }
                            title="Payment Methods (This Month)"
                            isEmpty={(paymentBreakdown ?? []).length === 0}
                        >
                        ...
                        <BreakdownPanel
                            icon={
                                <Users className="size-4 text-[var(--pos-teal)]" />
                            }
                            title="By Cashier (This Month)"
                            isEmpty={(cashierBreakdown ?? []).length === 0}
                        >
                        ...
                        <BreakdownPanel
                            icon={
                                <Trophy className="size-4 text-[var(--pos-green)]" />
                            }
                ...
                {canManage &&
                    (gcashOverview || payablesSummary || openShifts) && (
                        <div className="grid grid-cols-1 gap-3 sm:gap-4 lg:grid-cols-3">
                            <BreakdownPanel
                                icon={
                                    <Wallet className="size-4 text-[var(--pos-teal)]" />
                                }
                                title="GCash Float"
                            ...
                            >
                                {(gcashOverview ?? []).map((g) => (
                                    <div
                                        key={g.location_id}
                                        className="mb-2 border-b pb-2 last:mb-0 last:border-0 last:pb-0"
                                    >
                                        ...
                                        <div className="mt-0.5 flex justify-between text-xs text-muted-foreground">
                                            <span>
                                                Today: +{peso(g.today_cash_in)}{' '}
                                                in
                                            </span>
                                            <span>
                                                −{peso(g.today_cash_out)} out
                                            </span>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <BreakdownPanel
                                ...
                                isEmpty={
                                    !payablesSummary ||
                                    payablesSummary.unpaid_count === 0
                                }
                            >
                                {payablesSummary && (
                                    <>
                                        <div className="mb-3 flex items-baseline justify-between">
                                            <span className="font-mono text-lg font-bold tabular-nums">
                                                {peso(
                                                    payablesSummary.total_unpaid,
                                                )}
                                            </span>
                                            <span className="text-xs text-muted-foreground">
                                                {payablesSummary.unpaid_count}{' '}
                                                unpaid
                                            </span>
                                        </div>
                                        ...
                                        <Link
                                            ...
                                            className="mt-2 block text-xs font-medium text-[var(--pos-teal)] hover:underline"
                                        >
                                            View all &rarr;
                                        </Link>
                                    </>
                            ...
                            <BreakdownPanel
                                icon={
                                    <UserCheck className="size-4 text-[var(--pos-green)]" />
                                }
                ...
                {isOwner && branchBreakdown && (
                    <div className="rounded-xl border bg-card p-4 shadow-sm">
                        <h2 className="mb-3 text-sm font-medium">
                            Branch Comparison — Profit &amp; Loss (This Month)
                        </h2>
                                ...
                                <tbody>
                                    {branchBreakdown.map((b) => (
                                        ...
                                        >
                                            ...
                                            <td className="p-2.5 text-right font-mono text-muted-foreground tabular-nums">
                                                {peso(b.cogs)}
                                            </td>
                                            <td className="p-2.5 text-right font-mono text-muted-foreground tabular-nums">
                                                {peso(b.expenses)}
    ...
    return (
        <div
            className={cn(
                ...
                highlight
                    ? 'bg-gradient-to-br from-[var(--pos-teal)]/10 via-[var(--pos-teal)]/5 to-transparent'
                    : 'bg-card',
            )}
        >
            <p className="text-xs text-muted-foreground">{label}</p>
