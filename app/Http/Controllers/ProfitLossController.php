...
class ProfitLossController extends Controller
{
    ...
    /**
     * Periodic-method reconciliation: Beginning Inventory + Purchases − Ending
     * Inventory, cross-checked against actual (perpetual) COGS to surface any
     * shrinkage. See ProfitLossService::reconciliation() for how the figures
     * are derived from finalized physical inventory counts.
     */
    public function reconciliation(Request $request): Response
    {
        ...
        return Inertia::render('reports/reconciliation', [
            'reconciliation' => $this->profitLoss->reconciliation($startDate, $endDate, $locationId),
            'startDate' => $startDate,
            'endDate' => $endDate,
            'isOwner' => $isOwner,
            'locations' => $isOwner ? Location::where('is_active', true)->orderBy('name')->get(['id', 'name']) : [],
